...
const file = ref(null)
const splitMode = ref('all') // 'all' | 'range'
...
const resetState = () => {
  ...
  rangeInput.value = ''
  splitMode.value = 'all'
  // Pages will update via watcher or load
}

// Watch file change to get page count from preview component eventually,
// or we load it ourselves here to be safe and fast?
// PdfPreview loads it. Let's rely on PdfPreview exposing it or loading it.
// Actually, for robust state, let's load it here sparingly or via PdfPreview event?
// Let's use a simple watcher on file using pdf-lib or let the preview component tell us.
// Ideally, loading it once is better. PdfPreview uses pdfjs, we use pdf-lib for splitting.
// We can use pdf-lib here to get page count quickly.
...
// Update start/end pages based on range input
...
const parseRangeForPreview = () => {
  ...
  try {
    ...
    // Ignore invalid range during typing for preview
  }
}
...
const startOver = () => {
  showResult.value = false
  resultZip.value = null
  // Keep file, keep settings? User request said 'Split Again'
}
</script>
  ...
  <div class="w-full max-w-2xl px-4 py-8">
    ...
    <main class="glass p-8 md:p-10 relative">
      ...
      <div v-else class="animate-fade-in">
        ...
        <div v-if="!processing && !showResult" class="space-y-6">
          <div class="flex flex-col sm:flex-row justify-center gap-6">
            <label class="flex items-center gap-3 cursor-pointer group">
              <input type="radio" v-model="splitMode" value="all" class="w-5 h-5 accent-purple-500 cursor-pointer" />
              <span class="text-slate-200 group-hover:text-white transition-colors">All Pages</span>
            </label>
            <label class="flex items-center gap-3 cursor-pointer group">
              <input type="radio" v-model="splitMode" value="range" class="w-5 h-5 accent-purple-500 cursor-pointer" />
              <span class="text-slate-200 group-hover:text-white transition-colors">Custom Range</span>
            </label>
          </div>

          <div v-if="splitMode === 'range'" class="animate-slide-down">
            <input 
              v-model="rangeInput"
              type="text" 
              placeholder="e.g. 1-5, 8, 11-13" 
              class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-purple-500/50 focus:bg-white/10 transition-all font-mono text-sm"
            />
            <p class="text-xs text-slate-500 mt-2 ml-1">Extracts specified pages into individual files.</p>
          </div>
          ...
          <button 
            @click="processPDF"
            class="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl font-bold text-lg text-white shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 relative overflow-hidden group"
          >
